###############################################################################
## We check what happens with the simuls if using L=3 and just LMM (ie no
## mixed models, no binary outcome)
##############################################################################
#############################################################################
#############################################################################
##Libraries:
...
sigmaa <- matrix(c(runif(1,0.25,0.75), runif(6, -0.0005, 0.0005), 
                   runif(1,0,0.15), runif(6, -0.0005, 0.0005),
...
IBS_univ <- IBS_w  <- matrix(nrow = repl, ncol = 3)
IBS_univ_test  <- IBS_w_test <- matrix(nrow = repl, ncol = 3)
...
for(count in 1:repl){
  ...
  try(cl <- parallel::makeCluster(3L))
  ...
  if(count == repl){
    strr <- "rebuilding_simul_3outcomes_LMM_correl_typ1_SL_05mar2025.RData"
...
df_comparing_3_LMM_1 <- data.frame(IBS_mJM_train = ibs_train,
...
#boxplot IBS
df <- with(df_comparing_3_LMM, data.frame("dSL CV Data" = IBS_dSL_train,
