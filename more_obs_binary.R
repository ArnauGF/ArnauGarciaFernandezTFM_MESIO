...
DF <- data.frame(id = rep(seq_len(n), each = 2*K-1),
                 sex = rep(gl(2, n/2, labels = c("male", "female")), each = 2*K-1),
                 treat = treat,
                 time_compl = c(replicate(n, seq(from = 0, to = 9, by = 0.5))),
                 time = c(replicate(n, seq(from = 0, to = 9, by = 0.5))))

DF <- DF %>%
  mutate(
    time = ifelse(time %% 1 != 0, NA, time)
  )

DF_test <- data.frame(id = rep(seq_len(n_test), each = 2*K-1),
                      sex = rep(gl(2, n_test/2, labels = c("male", "female")), each = 2*K-1),
                      treat = treat,
                      time_compl = c(replicate(n, seq(from = 0, to = 9, by = 0.5))),
                      time = c(replicate(n, seq(from = 0, to = 9, by = 0.5))))
DF_test <- DF_test %>%
...
eta_y1 <- as.vector(X1 %*% betas1 + rowSums(Z1 * b1[na.omit(DF)$id, ]))
y1 <- rnorm(n * K, mean = eta_y1, sd = sigma1)
y1_matrix <- matrix(y1, nrow = n, ncol = 10, byrow = T)

DF$y1 <- fill(y1_matrix, seq(0, 9, 0.5))

#b1_test <- mvrnorm(n = n_test, mu = c(0,0), Sigma = matrix(c(0.067, 0.0025, 0.0000178, 0.0008988), 
#                                                 nrow = 2, ncol = 2))
b1_test <- b_test[, c(1,2)]

eta_y1_test <- as.vector(X1_test %*% betas1 + rowSums(Z1_test * b1_test[na.omit(DF_test)$id, ]))
y1_test <- rnorm(n * K, mean = eta_y1_test, sd = sigma1)
y1_test_matrix <- matrix(y1_test, nrow = n, ncol = K, byrow = T)

DF_test$y1 <- fill(y1_test_matrix, seq(from = 0, to = 9, by = 0.5))
...
##Longitudinal profile simulation
eta_y2 <- as.vector(X2 %*% betas2 + rowSums(Z2 * b2[na.omit(DF)$id, ]))
y2 <- rnorm(n * K, mean = eta_y2, sd = sigma2)
y2_matrix <- matrix(y2, nrow = n, ncol = K, byrow = T)
DF$y2 <- fill(y2_matrix, seq(0,9,0.5))

#b2_test <- mvrnorm(n = n_test, mu = c(0,0), Sigma = matrix(c(0.067, 0.00025, 0.00178, 0.0008988), 
#                                                           nrow = 2, ncol = 2))
b2_test <- b_test[, c(3,4)]

eta_y2_test <- as.vector(X2_test %*% betas2 + rowSums(Z2_test * b2_test[na.omit(DF_test)$id, ]))
y2_test <- rnorm(n * K, mean = eta_y2_test, sd = sigma2)
y2_test_matrix <- matrix(y2_test, nrow = n, ncol = K, byrow = T)
DF_test$y2 <- fill(y2_test_matrix, seq(0,9,0.5))
...
##Longitudinal profile simulation
eta_y3 <- as.vector(X3 %*% betas3 + rowSums(Z3 * b3[na.omit(DF)$id, ]))
y3 <- rnorm(n * K, mean = eta_y3, sd = sigma2)
y3_matrix <- matrix(y3, nrow = n, ncol = K, byrow = T)
DF$y3 <- fill(y3_matrix, seq(0,9,0.5))


#b3_test <- mvrnorm(n = n_test, mu = c(0,0), Sigma = matrix(c(0.009123, 0.000025, 0.0005716, 0.00008988), 
#                                                           nrow = 2, ncol = 2))
b3_test <- b_test[, c(5,6)]
eta_y3_test <- as.vector(X3_test %*% betas3 + rowSums(Z3_test * b3_test[na.omit(DF_test)$id, ]))
y3_test <- rnorm(n * K, mean = eta_y3_test, sd = sigma3)
y3_test_matrix <- matrix(y3_test, nrow = n, ncol = K, byrow = T)
DF_test$y3 <- fill(y3_test_matrix, seq(0,9,0.5))
...
X4 <- model.matrix(~ sex + treat + time_compl, data = DF)
Z4 <- model.matrix(~ 1, data = DF)
X4_test <- model.matrix(~ sex + treat + time_compl, data = DF_test)
...
DF$eta_y4 <- eta_y4
DF$y4 <- rbinom(n*(2*K-1), size = 1, prob = plogis(eta_y4))
...
DF_test$y4 <- rbinom(n_test*(2*K-1), size = 1, prob = plogis(eta_y4_test))

# Let us see the longitudinal profiles
xyplot(y4 ~ time_compl, groups = id,
       data = DF,
...
xyplot(eta_y4 ~ time_compl, groups = id,
       data = DF,
...
xyplot(plogis(eta_y4) ~ time_compl, groups = id,
       data = DF,
       type = "l" ,xlab="Time",ylab="eta Y4")
...
DF_test <- DF_test[DF_test$time_compl <= DF_test$Time, ]
...
try(GLM4 <- mixed_model(y4 ~ sex + treat + time_compl, data = DF,
...
t0 <- 4.5
dt <- 1
try(brier_score_multi_train <- tvBrier(multiJM, newdata = na.omit(DF), Tstart = t0, Dt = dt, 
                                       integrated = TRUE))
try(brier_score_multi_test <- tvBrier(multiJM, newdata = na.omit(DF_test), Tstart = t0, Dt = dt, 
                                      integrated = TRUE))

### we obtian 0.5 of ibs approx, which is not that good
## we check what happens with another interval
t0 <- 5.5
dt <- 1
try(brier_score_multi_train2 <- tvBrier(multiJM, newdata = na.omit(DF), Tstart = t0, Dt = dt, 
                                       integrated = TRUE))
try(brier_score_multi_test2 <- tvBrier(multiJM, newdata = na.omit(DF_test), Tstart = t0, Dt = dt, 
                                      integrated = TRUE))

##we obtain approx 0.6 ibs with this interval, which is even worse
...
for(count in 1:repl){
  ...
  ##saving true times
  checkTimes_test[count] <- sum(trueTimes==150)/n_test
  ...
  if(count == repl){
    strr <- "rebuilding_simul_4outcomes_correl_typ1_02mar2025.RData"
