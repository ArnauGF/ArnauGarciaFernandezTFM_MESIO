...
###########################################################################
# First attempt
# - Six models as data-generating models (the multivariate JM and the 
#   L=5 univariate JM)
# - Indep random effects (between longitudinal outcomes and measurements)
# - Type 1 censoring


# - WARNING!!!: these simulations are not stable since trueTimes are bigger 
#   than 150 in more than 5% of cases. 
###########################################################################
library(JMbayes2)
num_dat <- 150
...
for(count in 1:num_dat){
  ...
  #if(count==1){
  #  strr <- "results1iter_Scenario2BIS.RData"
  ...
  #}
  #if(count==2){
  #  str1 <- "results2_Scenario2BIS.RData"
  #  save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
  #       dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
  #       IBS_multi_test, EPCE_multi_test, censoring_train,
  ...
  #}
  try(if(count==10){
    str10 <- "results10_Scenario2_3.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
...
#SCENARIO I
  ...
  # - Random censoring (C~N(6.5, 1))
  # - Non-indep random effects (but independent between longitudinal outcomes)
###########################################################################
###########################################################################
library(JMbayes2)
library(MASS)
...
for(count in 1:num_dat){
  ...
  # we simulate random effects
  b5 <- mvrnorm(n = n, mu = c(0,0), Sigma = matrix(c(sqrt(D11_5), D12_5, D21_5, sqrt(D22_5))
                                                   , nrow = 2))
  # linear predictor
  eta_y5 <- as.vector(X5 %*% betas5 + rowSums(Z5 * b5[DF$id, ]))
  # mean of the binomial distribution
  ...
  if(disSL_epce[count] == 1){
    ...
    try(EPCE_dSL_test <- tvEPCE(Models$M4, newdata = DF_test,
                                Tstart = t0, Dt = dt))
  } else if(disSL_epce[count] == 5){
    try(EPCE_dSL_test <- tvEPCE(Models$M5, newdata = DF_test,
                                Tstart = t0, Dt = dt))
  }
...
for(count in 1:num_dat){
  ...
  try(multiJM <- jm(CoxFit, list(LM1, LM2, LM3, LM4, LM5), time_var = "time",
                    which_independent = "all",
                    n_iter = 12000L, n_burnin = 2000L, n_thin = 5L))
  #We are using which_independent = "all", we shouldn't use it!!!
  cat("Multi JM fitted\n")
...
# - Type 1 censoring 
# - Non-indep random effects (non-indep between longitudinal outcomes)
# - We fit the multi JM withiut which_independent = "all" in order to use
#   non-indep between longitudinal outcomes.
###########################################################################
###########################################################################
library(JMbayes2)
library(MASS)
...
for(count in 1:num_dat){
  ...
  try(if(count==60){
    str60 <- "results60_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==70){
    str70 <- "results70_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==80){
    str80 <- "results80_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
         checkTimes, checkTimes2, file=str80)
  }
  if(count==90){
    str90 <- "results90_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
         checkTimes, checkTimes2, file=str90)
  }
  if(count==100){
    str100 <- "results100_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==120){
    str120 <- "results120_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==150){
    str150 <- "results150_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==175){
    str175 <- "results175_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
  ...
  if(count==199){
    str200 <- "results200_sce3_13nov.RData"
    save(IBS_multi, EPCE_multi, IBS_univ, EPCE_univ, IBS_w, EPCE_w, dSL_cv_IBS,
         dSL_cv_EPCE, eSL_cv_IBS, eSL_cv_EPCE, eSL_test_IBS,  eSL_test_EPCE, 
         IBS_multi_test, EPCE_multi_test, censoring_train,
         censoring_test, dSL_test_IBS, dSL_test_EPCE, disSL_ibs, disSL_epce,
