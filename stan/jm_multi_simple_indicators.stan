...
parameters {
  ...
  // indicator stuff
  real<lower=0, upper=1> pind; //sucess prob for indicators
}
...
model {
  ...
  
  {
    ...
    //log_haz_q = log_basehaz + e_eta_q + a_beta[1] * y1_eta_q + a_beta[2] * y2_eta_q;
  
    // Log hazard at event times only
    //log_haz_etimes = head(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q + a_beta[2] * y2_eta_q;, Nevents);
  
    // Log hazard at quadrature points only
    //log_haz_qtimes = tail(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q + a_beta[2] * y2_eta_q;, Npat_times_qnodes);
    ...
    target += log_mix(
      ...
      log_mix(
        pind,
        sum(head(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q + a_beta[2] * y2_eta_q, Nevents)) 
    - dot_product(qwts, exp(tail(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q + a_beta[2] * y2_eta_q, Npat_times_qnodes))), // ind[1] = 1, ind[2] = 1
        
        sum(head(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q, Nevents)) 
    - dot_product(qwts, exp(tail(log_basehaz + e_eta_q + a_beta[1] * y1_eta_q, Npat_times_qnodes)))     // ind[1] = 1, ind[2] = 0
      
      ),
      log_mix(
        pind,
        sum(head(log_basehaz + e_eta_q + a_beta[2] * y2_eta_q, Nevents)) 
    - dot_product(qwts, exp(tail(log_basehaz + e_eta_q + a_beta[2] * y2_eta_q, Npat_times_qnodes))),     // ind[1] = 0, ind[2] = 1
        
        sum(head(log_basehaz + e_eta_q, Nevents)) 
    - dot_product(qwts, exp(tail(log_basehaz + e_eta_q, Npat_times_qnodes)))         // ind[1] = 0, ind[2] = 0
    )
    );
